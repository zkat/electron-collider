...
impl Comparator {
    ...
    fn at_least(p: Predicate) -> Option<Self> {
        Comparator::new(Bound::Lower(p), Bound::upper())
    }

    fn at_most(p: Predicate) -> Option<Self> {
        Comparator::new(Bound::lower(), Bound::Upper(p))
    }
    ...
    fn allows_all(&self, other: &Comparator) -> bool {
        self.lower <= other.lower && other.upper <= self.upper
    }

    fn allows_any(&self, other: &Comparator) -> bool {
    ...
    fn intersect(&self, other: &Self) -> Option<Self> {
        ...
        Comparator::new(lower.clone(), upper.clone())
    }
    ...
    fn difference(&self, other: &Self) -> Option<Vec<Self>> {
        ...
        if let Some(overlap) = self.intersect(other) {
            ...
            if self.lower < overlap.lower && overlap.upper < self.upper {
                return Some(vec![
                    Comparator::new(self.lower.clone(), Upper(overlap.lower.predicate().flip()))
                        .unwrap(),
                    Comparator::new(Lower(overlap.upper.predicate().flip()), self.upper.clone())
                        .unwrap(),
                ]);
            }
...
impl Range {
    ...
    pub fn any() -> Self {
        Self {
            comparators: vec![Comparator::new(Bound::lower(), Bound::upper()).unwrap()],
        }
    }
...
fn many_predicates(input: &str) -> IResult<&str, Vec<Comparator>, SemverParseError<&str>> {
    context("many predicates", separated_list1(tag(" || "), predicates))(input)
}

fn predicates(input: &str) -> IResult<&str, Comparator, SemverParseError<&str>> {
    context(
...
fn wildcard(input: &str) -> IResult<&str, Comparator, SemverParseError<&str>> {
    context(
        "wildcard",
        map_opt(x_or_asterisk, |_| {
            Comparator::at_least(Predicate::Including((0, 0, 0).into()))
        }),
    )(input)
}
...
fn any_operation_followed_by_version(
    input: &str,
) -> IResult<&str, Comparator, SemverParseError<&str>> {
    use Operation::*;
    context(
        ...
        map_opt(
            ...
            |parsed| match parsed {
                (GreaterThanEquals, (major, minor, patch, _, _)) => Comparator::at_least(
                ...
                (GreaterThan, (major, Some(minor), Some(patch), pre_release, build)) => {
                    Comparator::at_least(Predicate::Excluding(Version {
                        major,
                        minor,
                        patch,
                        pre_release,
                ...
                (GreaterThan, (major, Some(minor), None, _, _)) => {
                    Comparator::at_least(Predicate::Including((major, minor + 1, 0).into()))
                }
                (GreaterThan, (major, None, None, _, _)) => {
                    Comparator::at_least(Predicate::Including((major + 1, 0, 0).into()))
                }
                (LessThan, (major, Some(minor), None, _, _)) => {
                    Comparator::at_most(Predicate::Excluding((major, minor, 0, 0).into()))
                }
                (LessThan, (major, minor, patch, _, _)) => Comparator::at_most(
                    Predicate::Excluding((major, minor.unwrap_or(0), patch.unwrap_or(0)).into()),
                ),
                (LessThanEquals, (major, minor, None, _, _)) => Comparator::at_most(
                    Predicate::Including((major, minor.unwrap_or(0), 0, 0).into()),
                ),
                (LessThanEquals, (major, Some(minor), Some(patch), _, _)) => {
                    Comparator::at_most(Predicate::Including((major, minor, patch).into()))
                }
                (Exact, (major, Some(minor), Some(patch), _, _)) => {
                    Comparator::exact((major, minor, patch).into())
                }
...
fn x_and_asterisk_version(input: &str) -> IResult<&str, Comparator, SemverParseError<&str>> {
    context(
        "minor X patch X",
        map(
            tuple((
...
fn caret(input: &str) -> IResult<&str, Comparator, SemverParseError<&str>> {
    context(
        "caret",
        map_opt(
            ...
            |parsed| match parsed {
                (0, None, None, _, _) => {
                    Comparator::at_most(Predicate::Excluding((1, 0, 0, 0).into()))
                }
                (0, Some(minor), None, _, _) => Comparator::new(
                    Bound::Lower(Predicate::Including((0, minor, 0).into())),
                    Bound::Upper(Predicate::Excluding((0, minor + 1, 0, 0).into())),
                ),
                // TODO: can be compressed?
                (major, None, None, _, _) => Comparator::new(
                    Bound::Lower(Predicate::Including((major, 0, 0).into())),
                    Bound::Upper(Predicate::Excluding((major + 1, 0, 0, 0).into())),
                ),
                (major, Some(minor), None, _, _) => Comparator::new(
                    Bound::Lower(Predicate::Including((major, minor, 0).into())),
                    Bound::Upper(Predicate::Excluding((major + 1, 0, 0, 0).into())),
                ),
                (major, Some(minor), Some(patch), pre_release, _) => Comparator::new(
...
fn tilde(input: &str) -> IResult<&str, Comparator, SemverParseError<&str>> {
    context(
        "tilde",
        map_opt(tuple((tilde_gt, partial_version)), |parsed| match parsed {
            (Some(_gt), (major, None, None, _, _)) => Comparator::new(
                Bound::Lower(Predicate::Including((major, 0, 0).into())),
                Bound::Upper(Predicate::Excluding((major + 1, 0, 0, 0).into())),
            ),
            (Some(_gt), (major, Some(minor), Some(patch), _, _)) => Comparator::new(
                Bound::Lower(Predicate::Including((major, minor, patch).into())),
                Bound::Upper(Predicate::Excluding((major, minor + 1, 0, 0).into())),
            ),
            (None, (major, Some(minor), Some(patch), _, _)) => Comparator::new(
                Bound::Lower(Predicate::Including((major, minor, patch).into())),
                Bound::Upper(Predicate::Excluding((major, minor + 1, 0, 0).into())),
            ),
            (None, (major, Some(minor), None, _, _)) => Comparator::new(
                Bound::Lower(Predicate::Including((major, minor, 0).into())),
                Bound::Upper(Predicate::Excluding((major, minor + 1, 0, 0).into())),
            ),
            (None, (major, None, None, _, _)) => Comparator::new(
                Bound::Lower(Predicate::Including((major, 0, 0).into())),
                Bound::Upper(Predicate::Excluding((major + 1, 0, 0, 0).into())),
            ),
...
fn hyphenated_range(input: &str) -> IResult<&str, Comparator, SemverParseError<&str>> {
    context(
...
fn no_operation_followed_by_version(
    ...
    context(
        ...
        map_opt(partial_version, |parsed| match parsed {
            ...
            (major, maybe_minor, _, _, _) => Comparator::new(
    ...
    */
    ...
    fn serialize_a_versionreq_to_string() {
        let output = serde_json::to_string(&WithVersionReq {
            req: Range {
                comparators: vec![Comparator::at_most(Predicate::Excluding(
                    "1.2.3".parse().unwrap(),
                ))
                .unwrap()],
            },
